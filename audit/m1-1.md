...
# Check recency: at least one <lastmod> within last 90 days
python3 -c '
import re, datetime
try: body=open("/tmp/sitemap.xml").read()
except: print("sitemap_recent N/A"); raise SystemExit
dates=re.findall(r"<lastmod>(\d{4}-\d{2}-\d{2})", body)
if not dates: print("sitemap_recent no_dates"); raise SystemExit
newest=max(dates)
delta=(datetime.date.today()-datetime.date.fromisoformat(newest)).days
print("sitemap_newest", newest, "days_ago", delta, "recent" if delta<=90 else "STALE")
' 2>/dev/null
...
| 9 | Sitemap carries recent `<lastmod>` | `/sitemap.xml` body contains ≥ 1 `<lastmod>…</lastmod>` element (`sitemap_lastmod present`) with at least one date within the last 90 days - a stale sitemap signals abandoned content | 1 |
| 10 | Modular llms.txt | At least one per-area variant (`/docs/llms.txt`, `/api/llms.txt`, `/developers/llms.txt`) returns HTTP 200 and is not an HTML soft-404 | 0 |

Status: **Pass** ≥ 8/9 · **Partial** 3-7/9 · **Fail** 0-2/9. (Sub-check 10 is weight 0 - a bonus signal with no lift evidence yet.)

...
# ARD ai-catalog.json (Google Agentic Resource Discovery, Jun 2026)
curl -fsS "$ORIGIN/.well-known/ai-catalog.json" -o /tmp/aicatalog.json -w 'ai-catalog %{http_code}\n' 2>/dev/null
python3 - <<'PY'
import json
try: d=json.load(open("/tmp/aicatalog.json"))
except Exception: print("ai_catalog_parse FAILED"); raise SystemExit
caps=d.get("capabilities") or d.get("services") or []
rq=d.get("representativeQueries") or []
print("ai_catalog_valid", isinstance(d,dict) and len(caps)>=1)
print("ai_catalog_capabilities", len(caps))
print("ai_catalog_queries", len(rq), "present" if rq else "absent")
PY
```
...
| 7 | (Bonus) DNS-AID record published | `_index._agents.<domain>` returns ≥ 1 SVCB answer (the org agent index), or a `_agents-challenge.<domain>` TXT is present. DNSSEC-authenticated (`AD=true`) is a bonus, never required | 0 |
| 8 | `/.well-known/ai-catalog.json` (ARD) | HTTP 200, valid JSON, declares ≥ 1 capability. `representativeQueries` present is a bonus. | 1 |

Status: **Pass** ≥ 6/7 · **Partial** 3-5/7 · **Fail** 0-2/7.

> Sub-check 6 is additive: a site passes on the classic well-known files alone. Sub-check 7 (DNS-AID) is weight 0 (matching m3-2) - treat its presence as a forward-looking signal, never penalise its absence. Sub-check 8 (`ai-catalog.json`) reflects Google ARD, launched Jun 17 2026 with broad coalition backing.
